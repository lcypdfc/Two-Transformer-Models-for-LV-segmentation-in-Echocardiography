_base_ = [
    '../mmsegmentation/configs/knet/knet_s3_upernet_swin-l_8x2_640x640_adamw_80k_ade20k.py'
]
...
model = dict(
    decode_head=dict(
        kernel_generate_head=dict(num_classes=2),
    ),
    auxiliary_head=dict(num_classes=2, norm_cfg=norm_cfg))

custom_imports = dict(imports=['echods.seg', 'mmseg.models.decode_heads'], allow_failed_imports=False)
